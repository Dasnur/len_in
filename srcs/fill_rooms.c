...
void	fill_room(char *line, room *room)
{
	size_t		i;
...
void    fill_rooms(farm *farm)
{
	...
	while (i < farm->count_of_rooms + 2)
	{
		...
		else
		{
			...
			fill_room(tmp_map->line, farm->rooms[i]);
			i++;
			tmp_map = tmp_map->next;
		}
	}
